#!/usr/bin/env python3
"""Alternate scraper: Sanskrit (Devanagari) + translation + commentary into
data_github/chapter-NN.json, one file per chapter, one record per verse.

This is NOT what currently feeds index.html — see scraper.py for that
(Swami Sivananda's translation, from dlshq.org, run locally). This script
is kept because it's the only scraper that can actually run inside a
Claude Code session: dlshq.org (and every other Gita site tried) is
blocked by this sandbox's network egress policy, but raw.githubusercontent.com
is reachable, so this pulls from the vedicscriptures/bhagavad-gita dataset
published there instead. Each chapter and each verse is served as its own
JSON resource (chapter/bhagavadgita_chapter_<n>.json and
slok/bhagavadgita_chapter_<n>_slok_<v>.json), fetched and parsed
individually — the same way it would page through per-verse HTML on any
other Gita site. BeautifulSoup is used in clean_text() to strip stray
markup some commentary fields carry.
...
To switch the reader back to this source: run this script, then copy
data_github/*.json over data/*.json (schemas match — both include
transliteration and Devanagari, unlike scraper.py's dlshq-sourced data).
"""
import json
import os
import re
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data_github")
